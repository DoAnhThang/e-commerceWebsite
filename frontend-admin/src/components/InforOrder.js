...
function InforOrder({ order, setViewOrder }) {
  return (
    <>
      {ReactDOM.createPortal(
        <div
          ...
          onClick={() => setViewOrder({ isView: false, order: {} })}
        />,
      ...
      {ReactDOM.createPortal(
        ...
        >
          <div
            ...
            onClick={() => setViewOrder({ isView: false, order: {} })}
          >
            <i className="fa-solid fa-xmark fs-4 text-secondary text-shadow-hover cursor-pointer p-2"></i>
          </div>

          <div className="text-secondary fs-7 mb-5 mt-4">
            <h3 className="text-dark">INFORMATION ORDER</h3>
            <p className="mb-1">ID User: {order.userId._id}</p>
